...
def parse_factor(tokens: []):
	...
	elif token_type == calc_lexer.TOKEN_EOF:
...
def parse_factor_paren(tokens: []):
	(token_type, token) = tokens.pop(0);
	# group begins with '('
	if token_type != calc_lexer.TOKEN_GROUP or token != '(':
...
def parse_ident(tokens: []):
	(token_type, token) = tokens.pop(0);
	if token_type == calc_lexer.TOKEN_IDENT:
...
def parse_infix_list(tokens: [], sides, op_token: str):
	...
	while True:
		(token_type, token) = tokens.pop(0);
		if token_type != calc_lexer.TOKEN_OP or not token in op_token:
			tokens.insert(0, (token_type, token));
			break;
...
def is_end_paren(token_type, token):
	return token_type == calc_lexer.TOKEN_GROUP and token == ')';

def is_comma(token_type, token):
	return token_type == calc_lexer.TOKEN_COMMA;
